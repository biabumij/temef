...
class Laporan extends Secure_Controller {
	...
	public function cetak_penerimaan_pembelian_alat()
	{
		$this->load->library('pdf');
		...
		if(!empty($date)){
			...
			if($query->num_rows() > 0){
				...
				foreach ($query->result_array() as $key => $sups) {
					...
					$mats = array();
					$materials = $this->pmm_model->GetReceiptMatAlatPrint($sups['supplier_id'],$purchase_order_no,$start_date,$end_date,$filter_material);
					if(!empty($materials)){
						foreach ($materials as $key => $row) {
							$arr['no'] = $key + 1;
							$arr['measure'] = $row['measure'];
	        ...
	        $pdf->SetTitle('BBJ - Laporan Pembelian Alat');
	        $pdf->nsi_html($html);
	        $pdf->Output('laporan-pembelian-alat.pdf', 'I');
	...
	public function cetak_hutang_penerimaan_alat()
	{
		$this->load->library('pdf');
		...
		$query = $this->db->get('pmm_purchase_order ppo');
			...
			$data['grand_total_pembayaran'] = $grand_total_pembayaran;
			$data['grand_total_hutang'] = $grand_total_hutang;
			$html = $this->load->view('laporan_pembelian/cetak_hutang_penerimaan_alat',$data,TRUE);

	        
	        $pdf->SetTitle('BBJ - Laporan Hutang Alat');
	        $pdf->nsi_html($html);
	        $pdf->Output('laporan-hutang-alat.pdf', 'I');
