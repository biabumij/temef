...
class Reports extends CI_Controller {
	...
	public function beban_pokok_produksi($arr_date)
	{
		...
		<style type="text/css">
			...
			$truck_mixer = $truck_mixer['nilai'];

			$wheel_loader = $this->db->select('SUM(prm.display_price) as nilai')
			->from('pmm_receipt_material prm')
			->join('pmm_purchase_order po', 'prm.purchase_order_id = po.id','left')
			->where("prm.date_receipt between '$date1' and '$date2'")
			->where("prm.material_id in (16)")
			->where("po.status in ('PUBLISH','CLOSED')")
			->get()->row_array();

			$wheel_loader = $wheel_loader['nilai'];

			$bbm_solar = $this->db->select('SUM(prm.display_price) as nilai')
			->from('pmm_receipt_material prm')
			->join('pmm_purchase_order po', 'prm.purchase_order_id = po.id','left')
			->where("prm.date_receipt between '$date1' and '$date2'")
			->where("prm.material_id in (8)")
			->where("po.status in ('PUBLISH','CLOSED')")
			->get()->row_array();

			$bbm_solar = $bbm_solar['nilai'];
